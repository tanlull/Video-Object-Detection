...
def execute_graph(gdef, dumm_inp):
    ...
    with csess.Session(config=sessconfig, graph=g) as sess:
        ...
        print("Predict time average %f seconds" %((keeptime/10.0)))
        for top in sorted_result[:1]:
            print("Predict class : "+imagenet_label[top[0]]+"with confidence: %.3f" % top[1])
    return val , keeptime

       
def get_yolo_boxes_rt(model, images, net_h, net_w, anchors, obj_thresh, nms_thresh):
    ...
    batch_output , predict_time = execute_graph(model, batch_input)
